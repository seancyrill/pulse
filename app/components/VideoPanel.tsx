...
export default function VideoPanel({
  ...
  useEffect(() => {
    if (!speakerSupported || !selectedSpeakerId || !remoteRef.current) return
    void setAudioOutput(remoteRef.current, selectedSpeakerId).catch(() => {
      setDeviceError("Couldn't switch speaker.")
    })
  }, [remoteStream, selectedSpeakerId, speakerSupported])
  ...
  async function handleMicChange(deviceId: string) {
    if (!deviceId || deviceId === selectedMicId || switchingMic) return
    setDeviceError(null)
  ...
  async function handleSpeakerChange(deviceId: string) {
    if (!deviceId || !remoteRef.current) return
    setDeviceError(null)
    setSelectedSpeakerId(deviceId)
    try {
      await setAudioOutput(remoteRef.current, deviceId)
    } catch {
      setDeviceError("Couldn't switch speaker.")
    }
  }
      ...
      <div className="relative flex-1 w-full max-w-2xl mx-auto bg-zinc-900">
        ...
        <div className="absolute bottom-4 left-0 right-0 z-10 flex justify-center gap-4">
          ...
          <button
            ...
            className={`flex h-13 w-13 items-center justify-center rounded-full ${
              settingsOpen ? "bg-emerald-500/30" : "bg-white/15"
            } text-white`}
          >
            <SettingsIcon />
          </button>
